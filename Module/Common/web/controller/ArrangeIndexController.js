...
$(function () {
    ...
    cols.push({
        field: "SOLUTION_NAME", title: '方案名称', groupHeaderTemplate: '方案名称: <span>#= value #</span>' +
        '<button class="btn purple" value="#=value#" onclick="return toolbar_editSolution(this)" style="margin-left: 4px;">编辑<i class="icon-edit"></i></button>' +
        '<button class="btn red" value="#=value#" onclick="return toolbar_deleteSolution(this)" style="margin-left: 4px;">删除<i class="icon-remove-sign"></i></button>', width: 80, sortable: true, filterable: false, hidden: true
    });
    cols.push({ field: "SHIFT_NAME", title: '班次名称', width: 80, sortable: true, filterable: false });
    cols.push({ field: "START_DATE", title: '开始时间', width: 80, format: "{0: HH:mm}", sortable: true, filterable: false });
    cols.push({ field: "END_DATE", title: '结束时间', width: 80, format: "{0: HH:mm}", sortable: true, filterable: false });
    cols.push({ field: "VALID_TIME", title: '有效时间', width: 80, sortable: true, filterable: false, template: '#: parseFloat((VALID_TIME/3600).toFixed(1)) #' });
    //Grid
    grid = $("#grid").grid({
        //checkBoxColumn: true,
        ...
        save: function (e) {
            ...
            for (var i = 0; i < e.SHIFTS().length; i++) {
                ...
                if (e.SHIFTS()[i].SHIFT_NAME() == "") {
                    ...
                    $("#SHIFT_ERROR").addClass("error").html('内容不能为空').show();
                    return;
                }
            }
            if (parseFloat(TotalTime) > 24.0) {
                $("#SHIFT_ERROR").addClass("error").html('有效时间不能大于24个小时！').show();
                return;
            }
            if (validator.form()) {
                var shifts = [];
                ...
                if (parseInt(data[i].END_DATE().split(":")[0]) == 0) {
                    if (data[j].START_DATE() >= data[i].START_DATE()) {
                        if (parseInt(data[j].END_DATE().split(":")[0]) < 24 || (parseInt(data[j].END_DATE().split(":")[0]) == 0 && parseInt(data[j].END_DATE().split(":")[1]) < parseInt(data[i].END_DATE().split(":")[1]))) {
                            ...
                            $("#SHIFT_ERROR").addClass("error").html('班次不可有重叠！').show();
                            return data[j];
                        }
                    }
