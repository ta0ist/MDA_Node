...
function getPlanByMachine(e) {
    ...
    if (flag == 0) { //设备组
        ...
        $("#machinePlan").html(lang.Common.SchedulingPlan + "[" + name + "]");
            ...
            if (data.Status == 0) {
                ...
                for (var i = 0; i < data.Data.length; i++) {
                    ...
                    if (data.Data[i].solutionlist[0].CYCLE_TYPE == 0) {
                        shtml = shtml + lang.Common.NoCycle + '</h2><div class="timeline-content"><table class="table-bordered">';
                        for (var j = 0; j < data.Data[i].solutionlist.length; j++) {
                            ...
                            for (var m = 0; m < data.Data[i].solutionlist[j].li.length; m++) {
                                ...
                                shtml = shtml + '<td>' + parseFloat((data.Data[i].solutionlist[j].li[m].VALID_TIME / 3600).toFixed(1)) + lang.EmployeePerformance.Hours + '</td></tr>';
                            }
                            shtml = shtml + '</table></td></tr>';
                        }
                        shtml = shtml + '</table>';
                    } else if (data.Data[i].solutionlist[0].CYCLE_TYPE == 1) {
                        shtml = shtml + lang.Common.WeeklyCycle + '</h2><div class="timeline-content"><table class="table-bordered">';
                        for (var j = 0; j < data.Data[i].solutionlist.length; j++) {
                            ...
                            var week = [lang.Common.Mon, lang.Common.Tue, lang.Common.Wed, lang.Common.Thu, lang.Common.Fri, lang.Common.Sat, lang.Common.Sun];
                            for (var n = 0; n < 7; n++) {
                                if (weekday[n] == 1) {
                        ...
                        shtml = shtml + lang.Common.MonthlyCycle + '</h2><div class="timeline-content"><table class="table-bordered">';
                        for (var j = 0; j < data.Data[i].solutionlist.length; j++) {
                    ...
                    if (data.Data[i].ShiftType == "0") {
                        ...
                        shtml = shtml + '</div>' +
                            ...
                            lang.Order.Delete + ' <i class="icon-remove-sign m-icon-white"></i>' +
                            '</a>' +
                            ...
                            lang.Order.Edit + ' <i class="icon-edit m-icon-white"></i>' +
                            '</a>' +
                            '</div>' +
                            '</div>' +
                            '</li>';
...
function onChange() {
    ...
    if (CYCLE_TYPE == 1) { //周循环
        ...
        var head = ["", lang.Common.DatePlan, lang.Common.Mon, lang.Common.Tue, lang.Common.Wed, lang.Common.Thu, lang.Common.Fri, lang.Common.Sat, lang.Common.Sun];
...
function deleteplan(e) {
    BzConfirm(lang.Common.DeleteData, function(d) {
        if (d) {
            var treeobj = $("#orgnizetree").data("kendoTreeView");
...
function addOrEdit(id, shifttype, attr_guid) {
    ...
    $("#SOLUTIONS").kendoMultiSelect({
        ...
        headerTemplate: '<div class="dropdown-header">' +
            '<span class="k-widget k-header">' + lang.Common.PackageName + '</span>' +
            '<span class="k-widget k-header">' + lang.Common.ShiftName + '</span>' +
            '<span class="k-widget k-header">' + lang.EmployeePerformance.StartTime + '</span>' +
            '<span class="k-widget k-header">' + lang.EmployeePerformance.EndTime + '</span>' +
            '<span class="k-widget k-header">' + lang.Common.ValidTime + '</span>' +
            '</div>',
    ...
    $("#Add").bind("click", { ID: id, attr_guid: attr_guid }, function(event) {
        ...
        if ($("#START_DATE").data("kendoDatePicker").value() == null) {
            ...
            $("#SHIFT_ERROR").html(lang.Common.StartTimeCannotBeEmpty);
            return;
        }
        if ($("#END_DATE").data("kendoDatePicker").value() != null) {
            if (moment($("#END_DATE").data("kendoDatePicker").value()).isBefore($("#START_DATE").data("kendoDatePicker").value())) { // true)
                ...
                $("#SHIFT_ERROR").html(lang.Common.EndTimeIsNotLessThanStartTime);
                return;
            }
        } else {
            BzAlert(lang.Common.PleaseInputTheCorrectDate);
            return;
        }
        ...
        if (SOLUTIONS.length == 0) {
            ...
            $("#SHIFT_ERROR").html(lang.Common.PleaseSelectAFlightPlan);
            return;
        }
        ...
        if (data.machineId.length == 0) {
            $("#SHIFT_ERROR").html(lang.Common.PleaseSelectANeedToAssociatedEquipment);
            return;
        }
