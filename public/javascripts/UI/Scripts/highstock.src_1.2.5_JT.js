...
(function () {
 ...
 function getTimeTicks2(normalizedInterval, min, max, startOfWeek) {
        var tickPositions = [],
		i,
		higherRanks = {},
		useUTC = defaultOptions.global.useUTC,
       ...
       if (defined(min)){
interval=mathRound((max-min)/count/1000)*1000;
            // push the last time
            for (var i=0;i<count-1;i++){
            	tickPositions.push(min+(i+1)*interval);
            }
...
t5 = new Date().getTime();
            ...
            if (!tickPositions) {
                if (isDatetimeAxis) {
                    tickPositions = (axis.getNonLinearTimeTicks || getTimeTicks)( //axis.getNonLinearTimeTicks ||getTimeTicks
...
t6 = new Date().getTime();
       ...
       t7 = new Date().getTime();
                 //$("#kk").append("[ setTickPositions8()" + (t2 - t1).toString() + "," + (t3 - t2).toString() + "," + (t4 - t3).toString() + "," + (t5 - t4).toString() + "," + (t6 - t5).toString() + "," + (t7 - t6).toString() + "," + (t8 - t7).toString() + "," + (t9 - t8).toString() + "]");
        ...
        */
        setScale: function () {
            ...
            axis.setMaxTicks();
            t6 = new Date().getTime();
          // $("#kk").append("[ axis.setScale()" + (t2 - t1).toString() + "," + (t3 - t2).toString() + "," + (t4 - t3).toString() + "," + (t5 - t4).toString() + "," + (t6 - t5).toString() + "]");
        },
        ...
        getAnchor: function (points, mouseEvent) {
            ...
            if (!ret) {
                each(points, function (point) {
                    yAxis = point.series.yAxis; //lkj--2012/8/29
                    plotX += point.plotX;
    ...
    Chart.prototype = {
        ...
        */
        redraw: function (animation) {
            ...
            t9 = new Date().getTime();
           // $("#kk").append("[ chart.redraw()" + (t2 - t1).toString() + "," + (t3 - t2).toString() + "," + (t4 - t3).toString() + "," + (t5 - t4).toString() + "," + (t6 - t5).toString() + "," + (t7 - t6).toString() + "," + (t8 - t7).toString() + "," + (t9 - t8).toString() + "]");
        },
  ...
  var t1, t2,t3;
            ...
            if (seriesMarkerOptions.enabled || series._hasPointMarkers) {
                ...
                t1 = new Date().getTime();  	
                    ...
                    if (enabled && plotY !== UNDEFINED && !isNaN(plotY)) {
						...
						.attr(pointAttr)
						.add(markerGroup);//markerGroup
						//
						
						 t2 = new Date().getTime();
						 // $("#kk").append("," + (t2 - t1).toString() );
                        }
        ...
        */
        render: function () {
            ...
            t9 = new Date().getTime();
            // $("#kk").append("[ s.render()" + (t2 - t1).toString() + "," + (t3 - t2).toString() + "," + (t4 - t3).toString() + "," + (t5 - t4).toString() + "," + (t6 - t5).toString() + "," + (t7 - t6).toString() + "," + (t8 - t7).toString() + "," + (t9 - t8).toString() + "]");
        ...
        */
        redraw: function () {
            ...
            series.render();
            t4 = new Date().getTime();
           // $("#kk").append("[ series.translate()" + (t2 - t1).toString() + "," + (t3 - t2).toString() + "," + (t4 - t3).toString() + "]");
            if (wasDirtyData) {
 ...
 */
    ...
    * 
    **************************************************************************** */
    var JTLineSeries = extendClass(Series, {
        type: 'jtline',
        ...
        translate: function () {
            ...
            xlocalMin = xAxis.old ? xAxis.oldMin : xAxis.min,
            xlocalA = xAxis.old ? xAxis.oldTransA : xAxis.transA,

 ylocalMin = yAxis.old ? yAxis.oldMin : yAxis.min,
            ylocalA = yAxis.old ? yAxis.oldTransA : yAxis.transA,

			points = [];
			var lastPlotx,lastPloty;
            for (var i = 0; i < dataLength; i++) {
                var point = {};
                var Plotx,Ploty;
                xValue = xData[i];
                yValue = yData[i];
				Plotx=(xValue - xlocalMin) * xlocalA;
				Ploty=-(yValue - ylocalMin) * ylocalA + csvoff;
				if(i==0) {
					lastPlotx=Plotx;
					lastPloty=Ploty;
					point.plotX = Plotx;
					...
					if(mathAbs(Plotx-lastPlotx)>2||mathAbs(Ploty-lastPloty)>2){
						lastPlotx=Plotx;
					lastPloty=Ploty;
					point.plotX = Plotx;
					point.plotY=Ploty;
                ...
                // = -(yValue - ylocalMin) * ylocalA + csvoff; //mathRound(yAxis.translate(yValue, 0, 0, 0, 1) * 10) / 10;
        ...
        getSegmentPath: function (segment) {
			...
			segmentPath = [],
			step = series.options.step;
            dataLength = segment ? segment.length : 0;

            for (var i = 0; i < dataLength; i++) {
                segmentPath.push(i ? L : M);
                segmentPath.push(segment[i].plotX, segment[i].plotY);
            }
    ...
    seriesTypes.jtline = JTLineSeries;
    defaultPlotOptions.jtline = defaultSeriesOptions;
    /* */
    var AxixsProto = Axis.prototype;
    AxixsProto.getSeriesExtremes = function () {
        var axis = this,
			chart = axis.chart,
			stacks = axis.stacks,
			posStack = [],
        ...
        each(axis.series, function (series) {
            if (series.visible || !chart.options.chart.ignoreHiddenSeries) {
                ...
                if (axis.isXAxis) {
                    ...
                    xData = series.xData;
                    if (xData.length) {
                        var minmax = arrayMinMax(xData);
                        axis.dataMin = mathMin(pick(axis.dataMin, xData[0]), minmax[0]); //axis.dataMin = mathMin(pick(axis.dataMin, xData[0]), )
                        axis.dataMax = mathMax(pick(axis.dataMax, xData[0]), minmax[1]); //axis.dataMax = mathMax(pick(axis.dataMax, xData[0]), arrayMax(xData))
                    }
                    t2 = new Date().getTime();
                 // $("#kk").append("[ Axis.Extremes()" + (t2 - t1).toString() + "]");
                    // Get dataMin and dataMax for Y axes, as well as handle stacking and processed data
                } else {
                    var isNegative,
						pointStack,
                    ...
                    */
                    // Get the dataMin and dataMax so far. If percentage is used, the min and max are
                    // always 0 and 100. If the length of activeYData is 0, continue with null values.
                     activeYData=series.yData;
                    if (!axis.usePercentage && activeYData.length) {
                        var minmax = arrayMinMax(activeYData);
                        axis.dataMin = mathMin(pick(axis.dataMin, activeYData[0]), minmax[0]); // mathMin(pick(axis.dataMin, activeYData[0]), arrayMin(activeYData))
                        axis.dataMax = mathMax(pick(axis.dataMax, activeYData[0]), minmax[1]); //mathMax(pick(axis.dataMax, activeYData[0]), arrayMax(activeYData)
                    }
...
var pointer = 0, groupLength;
	groupLength = groupPositions.length;
	
        for (i = 0; i <= dataLength; i++) {

            // when a new group is entered, summarize and initiate the previous group
            while ((groupPositions[1] !== UNDEFINED && xData[i] >= groupPositions[pointer]&& pointer < groupLength) ||
                ...
                */
                xAxis.getNonLinearTimeTicks = function (normalizedInterval, min, max, startOfWeek, positions, closestDistance, findHigherRanks) {
                    ...
                    for (; end < posLength; end++) {
                        ...
                        if (end === posLength - 1 || positions[end + 1] - positions[end] > closestDistance * 100 || outsideMax) {
